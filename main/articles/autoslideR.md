...
`# load all filters`\
`filters``::`[`load_filters`](https://rdrr.io/pkg/filters/man/load_filters.html)`(``filters``, overwrite ``=`` ``TRUE``)`\
`# read data`\
...
`      DISTRTFL ``=`` `[`sample`](https://rdrr.io/r/base/sample.html)`(`[`c`](https://rdrr.io/r/base/c.html)`(``"Y"``, ``"N"``)``, size ``=`` `[`length`](https://rdrr.io/r/base/length.html)`(``TRT01A``)``, replace ``=`` ``TRUE``, prob ``=`` `[`c`](https://rdrr.io/r/base/c.html)`(``.1``, ``.9``)``)`\
`    ``)`` ``|>`\
...
`  `[`filter_spec`](https://pharmaverse.github.io/autoslider.core/reference/filter_spec.md)`(``program`` `[`%in%`](https://rdrr.io/r/base/match.html)` `[`c`](https://rdrr.io/r/base/c.html)`(`\
`    ``"t_ds_slide"``,`\
`    ``"t_dm_slide"`\
`  ``)``)`` ``|>`\
`  ``# these filtered specs are now piped into the generate_outputs function.`\
`  ``# this function also requires the data`\
`  `[`generate_outputs`](https://pharmaverse.github.io/autoslider.core/reference/generate_outputs.md)`(``datasets ``=`` ``data``)`` ``|>`\
...
`    outfile ``=`` `[`tempfile`](https://rdrr.io/r/base/tempfile.html)`(``fileext ``=`` ``".ppts"``)``,`\
`    template ``=`` `[`file.path`](https://rdrr.io/r/base/file.path.html)`(`[`system.file`](https://rdrr.io/r/base/system.file.html)`(``package ``=`` ``"autoslider.core"``)``, ``"/theme/basic.pptx"``)``,`\
...
\
`  ``adlb_f`` ``<-`` ``adlb`` ``|>`\
`    ``dplyr``::`[`filter`](https://dplyr.tidyverse.org/reference/filter.html)`(``ABLFL`` ``!=`` ``"Y"``)`` ``|>`\
`    ``dplyr``::`[`filter`](https://dplyr.tidyverse.org/reference/filter.html)`(``!``(``AVISIT`` `[`%in%`](https://rdrr.io/r/base/match.html)` `[`c`](https://rdrr.io/r/base/c.html)`(``"SCREENING"``, ``"BASELINE"``)``)``)`` ``|>`\
`    ``dplyr``::`[`mutate`](https://dplyr.tidyverse.org/reference/mutate.html)`(``AVISIT ``=`` `[`droplevels`](https://rdrr.io/r/base/droplevels.html)`(``AVISIT``)``)`` ``|>`\
`    ``formatters``::`[`var_relabel`](https://pharmaverse.github.io/formatters/latest-tag/reference/var_relabel.html)`(``AVISIT ``=`` ``"Visit"``)`\
\
`  ``adlb_f_crp`` ``<-`` ``adlb_f`` ``|>`` ``dplyr``::`[`filter`](https://dplyr.tidyverse.org/reference/filter.html)`(``PARAMCD`` ``==`` ``"CRP"``)`\
\
`  ``# Define the split function`\
`  ``split_fun`` ``<-`` ``rtables``::`[`drop_split_levels`](https://rdrr.io/pkg/rtables/man/split_funcs.html)\
\
`  ``lyt`` ``<-`` ``rtables``::`[`basic_table`](https://rdrr.io/pkg/rtables/man/basic_table.html)`(``show_colcounts ``=`` ``TRUE``)`` ``|>`\
`    ``rtables``::`[`split_cols_by`](https://rdrr.io/pkg/rtables/man/split_cols_by.html)`(``"ARM"``)`` ``|>`\
`    ``rtables``::`[`split_rows_by`](https://rdrr.io/pkg/rtables/man/split_rows_by.html)`(``"AVISIT"``,`\
...
`    alt_counts_df ``=`` ``adsl`\
`  ``)`` ``|>`\
`    ``rtables``::`[`trim_rows`](https://rdrr.io/pkg/rtables/man/trim_rows.html)`(``)`\
\
`  ``result`\
`}`
...
\
`  ``# join adsl to adlb`\
`  ``adlb`` ``<-`` ``adlb`` ``|>`` `[`semi_join`](https://dplyr.tidyverse.org/reference/filter-joins.html)`(``adsl``, by ``=`` ``"USUBJID"``)`\
\
`  ``# Please note that df_explict_na has a na_level argument defaulting to "<Missing>",`\
`  ``# Please don't change the na_level to anything other than NA, empty string or the default "<Missing>".`\
\
...
\
`lbt06``(`[`list`](https://rdrr.io/r/base/list.html)`(``adsl ``=`` ``adsl_f``, adlb ``=`` ``adlb_f_crp``)``)`\
`# Analysis Visit                                                                         `\
`#   Analysis Reference Range Indicator     A: Drug X        B: Placebo     C: Combination`\
`#     Baseline Status                       (N=134)          (N=134)          (N=132)    `\
`# ———————————————————————————————————————————————————————————————————————————————————————`\
...
`outputs`` ``<-`` ``spec_file`` ``|>`\
`  `[`read_spec`](https://pharmaverse.github.io/autoslider.core/reference/read_spec.md)`(``)`` ``|>`\
`  `[`generate_outputs`](https://pharmaverse.github.io/autoslider.core/reference/generate_outputs.md)`(``data``)`` ``|>`\
`  `[`decorate_outputs`](https://pharmaverse.github.io/autoslider.core/reference/decorate_outputs.md)`(``)`\
`# ``❯```  Running program `t_ds_slide` with suffix 'ITT'. ``\
`# Filter 'ITT' matched target ADSL.`\
`` # 400/400 records matched the filter condition `ITTFL == 'Y'`. ``\

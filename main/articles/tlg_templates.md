...
\
[`setwd`](https://rdrr.io/r/base/getwd.html)`(``"~/path/to/autoslider.core"``)`\
\
...
`                       ``arm`` ``=`` ``"TRT01P"``,`\
`                       ``vars`` ``=`` `[`c`](https://rdrr.io/r/base/c.html)`(``"AGE"``, ``"SEX"``, ``"RACE"``)``,`\
`                       ``stats`` ``=`` `[`c`](https://rdrr.io/r/base/c.html)`(``"median"``, ``"range"``, ``"count_fraction"``)``,`\
`                       ``split_by_study`` ``=`` ``FALSE``,`\
`                       ``side_by_side`` ``=`` ``NULL``)`` ``{`\
`  ``if`` ``(`[`is.null`](https://rdrr.io/r/base/NULL.html)`(``side_by_side``)``)`` ``{`\
`    ``extra`` ``<-`` ``NULL`\
`  ``}`` ``else`` ``{`\
`    ``extra`` ``<-`` `[`c`](https://rdrr.io/r/base/c.html)`(``"COUNTRY"``)`\
`  ``}`\
`  ``for`` ``(``v`` ``in`` `[`c`](https://rdrr.io/r/base/c.html)`(``vars``, ``extra``)``)`` ``{`\
`    ``assert_that``(`[`has_name`](https://rlang.r-lib.org/reference/has_name.html)`(``adsl``, ``v``)``)`\
`  ``}`\
`  ``adsl1`` ``<-`` ``adsl`` ``|>`\
`    `[`select`](https://dplyr.tidyverse.org/reference/select.html)`(`[`all_of`](https://tidyselect.r-lib.org/reference/all_of.html)`(`[`c`](https://rdrr.io/r/base/c.html)`(``"STUDYID"``, ``"USUBJID"``, ``arm``, ``vars``, ``extra``)``)``)`\
`  ``if`` ``(``!`[`is.null`](https://rdrr.io/r/base/NULL.html)`(``side_by_side``)``)`` ``{`\
`    ``adsl1``$``lvl`` ``<-`` ``"Global"`\
`  ``}`\
`  ``lyt`` ``<-`` `[`build_table_header`](https://pharmaverse.github.io/autoslider.core/reference/build_table_header.md)`(``adsl1``, ``arm``,`\
...
`      denom ``=`` ``"n"``,`\
`      vars ``=`` ``vars``,`\
`      .formats ``=`` `[`c`](https://rdrr.io/r/base/c.html)`(``mean_sd ``=`` ``"xx.xx (xx.xx)"``, median ``=`` ``"xx.xx"``)``,`\
`      var_labels ``=`` ``formatters``::`[`var_labels`](https://pharmaverse.github.io/formatters/latest-tag/reference/var_labels.html)`(``adsl1``)``[``vars``]`\
`    ``)`\
`  ``result`` ``<-`` `[`lyt_to_side_by_side`](https://pharmaverse.github.io/autoslider.core/reference/lyt_to_side_by_side.md)`(``lyt``, ``adsl1``, ``side_by_side``)`\
`  ``if`` ``(`[`is.null`](https://rdrr.io/r/base/NULL.html)`(``side_by_side``)``)`` ``{`\
`    ``# adding "N" attribute`\
`    ``arm`` ``<-`` ``col_paths``(``result``)``[[``1``]``]``[``1``]`\
`    ``n_r`` ``<-`` `[`data.frame`](https://rdrr.io/r/base/data.frame.html)`(`\
`      ARM ``=`` `[`toupper`](https://rdrr.io/r/base/chartr.html)`(`[`names`](https://rdrr.io/r/base/names.html)`(``result``@``col_info``)``)``,`\
`      N ``=`` ``col_counts``(``result``)`` ``|>`` `[`as.numeric`](https://rdrr.io/r/base/numeric.html)`(``)`\
`    ``)`` ``|>`\
`      ``` `colnames<-` ```(`[`c`](https://rdrr.io/r/base/c.html)`(`[`paste`](https://rdrr.io/r/base/paste.html)`(``arm``)``, ``"N"``)``)`` ``|>`\
`      ``dplyr``::`[`arrange`](https://dplyr.tidyverse.org/reference/arrange.html)`(`[`get`](https://rdrr.io/r/base/get.html)`(``arm``)``)`\
`    `[`attr`](https://rdrr.io/r/base/attr.html)`(``result``, ``"N"``)`` ``<-`` ``n_r`\
`  ``}`\
